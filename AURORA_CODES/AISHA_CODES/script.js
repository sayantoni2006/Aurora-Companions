// Aisha Chat JavaScript with Fixed Delete Functionality and Auto-Titling
document.addEventListener('DOMContentLoaded', function() {
    console.log('Aisha chat initialized');
    
    // DOM Elements
    const chatMessages = document.getElementById('aisha-chat-messages');
    const userInput = document.getElementById('aisha-user-input');
    const sendButton = document.getElementById('aisha-send-button');
    ...
    // Configuration
    const MAIN_WEBHOOK_URL = 'https://hook.eu2.make.com/zsn856ldighm3v314xk8xc0zrwk13jwb';
    const API_KEY = window.AISHA_API_KEY || 'Aisha Agent'; // Fallback if not set
    ...
    chats[currentChatId] = {
        messages: [
            {
                text: "Hello, I'm Aisha. I'm here to listen with an open heart. What would you like to share today? 💚",
                isUser: false,
                sender: 'Aisha',
                time: new Date()
            }
    ...
    function loadChatsFromStorage() {
        const savedChats = localStorage.getItem('aisha-chats');
        if (savedChats) {
            try {
    ...
    function saveChatsToStorage() {
        try {
            localStorage.setItem('aisha-chats', JSON.stringify(chats));
        } catch (e) {
    ...
    async function sendMessageToAisha(message) {
        try {
            ...
            if (detectCrisis(message)) {
                ...
                addMessage(crisisResponse, false, 'Aisha');
            ...
            if (replyText && replyText.trim() !== '') {
                ...
                addMessage(replyText, false, 'Aisha');
            } else {
            ...
            addMessage(simulatedResponse, false, 'Aisha');
        } finally {
    ...
    // Initialize the chat
    initChat();
    console.log('Aisha chat initialized successfully');
});